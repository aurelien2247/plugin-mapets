<!DOCTYPE html>
<html lang="fr">
<head>
	<title>Projet-Collier Pour Chien</title>
	<meta charset="utf-8">
	<link rel="stylesheet" href="/wp-content/themes/Projet-CollierPourChien/assets/css/style.css">
	<script src="https://kit.fontawesome.com/cfa8de8fe8.js" crossorigin="anonymous"></script>
</head>
<header>
	<!-- Haut de page -->
	<nav class="header">
		<img  class="logo" src="/wp-content/themes/Projet-CollierPourChien/image/logo.png">
		<?php connexion_utilisateur(); ?>
	</nav>

	<script type="text/javascript">
		const navSlide = () => {
			const burger = document.querySelector('.burger');
			const nav = document.querySelector('.nav-links');
			const navLinks = document.querySelectorAll('.nav-links li')
			
			burger.addEventListener('click',()=>{

				nav.classList.toggle('nav-active');

			navLinks.forEach((link, index) => {
				if(link.style.animation) {
					link.style.animation = '';
				} else {
					link.style.animation = `navLinkFade 0.5s ease forwards ${index / 7 +0.5}s`;
				}
			});
		});
	}
		navSlide();
	</script>
</header>
<body>
<script src="/wp-content/themes/Projet-CollierPourChien/assets/js/main.js"></script>


<?php
$user = wp_get_current_user();
if ( in_array( 'administrator', (array) $user->roles ) ) {
	// L'utilisateur actuel est administrateur
  echo '<link rel="stylesheet" href="/wp-content/plugins/admin_templates/css/custom.css">';
  echo '
  <style>
 .titre{
     color: #CFCFCF;
     font-size: 10px;
 }
 .nom{
  color: #CFCFCF;
}
.email{
 color: #CFCFCF;
}
.titre_de_tableau{
   padding-bottom: 3%;
   color: #CFCFCF;
   font-size: 39px;
   padding-top:0%;

}
.table{
 padding-top: 0%;

 width:100%
 border-collapse: collapse;
 color: #CFCFCF;
}
 td, th  Mettre une bordure sur les td ET les th {
     border: 1px solid #CFCFCF;
     border-radius: 1%
     color: #CFCFCF;
     width:10%;
     padding: 2%;


 }
  </style>


  <body>

    '. spacies() .
    '

  </body>
</div>';
} else {
	// L'utilisateur actuel n'est pas administrateur
	echo '
    <div id="acount-box">
        <h1>Erreur:</h1>
        <p class="inscription"> Afin de pouvoir avoir accès à cette page il est nécéssaire d\'avoir des roles d\'administrateur</a>.
        <br>Si vous pensez qu\'il y a erreur contacter l\'équipe de developpement</a>.
        <br><br> Cette page vous permeteras de pouvoir créer des nouvelles especes d\'animal.</p>
    </div>';
}

?>
</html>